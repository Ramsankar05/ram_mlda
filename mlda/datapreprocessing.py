...
data_preprocessed = preprocessor.fit_transform(data)print(data_preprocessed)
# Combine preprocessing steps of Output Data
preprocessor_Out = ColumnTransformer(
transformers=[
("num", num_transformer, ['Price (INR)'])])
preprocessor_Out.set_output(transform="pandas")# Apply the transformations to the Output data
data_preprocessed_Out =
preprocessor_Out.fit_transform(data)
...
# Display the first few rows of the processed
training data
print(X_train.head())
print(y_train.head())
T2: Simple Linear Regression
